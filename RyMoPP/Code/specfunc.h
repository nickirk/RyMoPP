...
void SpherYNoSin(int l, int m, double theta, double phi, double &ReY, double &ImY)
//---------------------------------------------------------------------------
// Evaluates the real and imaginary parts (ReY and ImY) of the spherical
// harmonic of orders l and m for arguments theta and phi, but without the sin(\theta) part
// use P^m_l(x)=(-1)^m(1-x^2)^{m/2}\frac{d^m}{dx^m}(P_l(x)) to calculate the apprximate result. remove (1-x^2)^{m/2} to remove sin(theta)\\
// this is only valid for m =1 or 2
//---------------------------------------------------------------------------
{
#define pi 3.141592653589793
   double fact;
   ...
   fact = 1e0;
   if (mabs==1) {
	Legendre(l,cos(theta),dmdxP_l);
	DxDP=dmdxP_l;
   }
   else {
	cout << "Specialfunc, SpherYNoSin, m should be 1! Fatal Error! Core Dumped!";
	abort();
   }
  //      Legendre(l,cos(theta)-deltax,dmdxP_l1)
  //      DxDP=(dmdxP_l-dmdxP_l1)/deltax;
  // }
   for (i=l-mabs+1; i<=l+mabs; i++) fact *= i;           // (l+|m|)!/(l-|m|)!

   fact = sqrt((2*l+1)/(4e0*pi*fact)) * DxDP;
   if (m < 0 && m % 2) fact = -fact;
