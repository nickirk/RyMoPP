...
MatrixXd OffDiag(vector<vector<int>>& WaveFuncsBasis, MatrixXd* WaveFuncs, MatrixXd mat, double x, int N, int DN, int DN1, int DL, bool FMagneticNumber, bool FPWave){
	int maxm=1, maxm1=1;
	double I=0.;
	int size = WaveFuncsBasis.size();
	int i;
...
int main(int argc, char const *argv[]) {
	int N=40, DN=3, DN1=0, DL=3,DM=1;//DM should smaller than l// maxm=1;
	int size=0,n,l,j;
	double xmin=48.6, xmax=52,I;
	double x=xmin, step=0.3;
	int loopnumber= int((xmax-xmin)/step);
	bool FReadH=false, FMagneticNumber=true, FPWave=false; // control the magnetic Number choices. In principle this option should always keep as true. Otherwise it means there is no spin orbital coupling, which doesn't make much sense.
	bool FManualSetBasis=false;
	string name0;
	omp_set_nested(1);
	omp_set_num_threads(3);
	vector<vector<int>> WaveFuncsBasis;
	vector<vector<int>> WaveFuncsBasistmp={};
	//WaveFuncsBasistmp={{37,5},{37,4}};
	...
	if (FManualSetBasis){
		for (int it=0; it < WaveFuncsBasistmp.size(); it++){
			int ltmp=WaveFuncsBasistmp[it][1];
			if (ltmp==0){
				int mtmp=1,djtmp=1;
				vector<int> Basistmp;
				Basistmp.push_back(WaveFuncsBasistmp[it][0]);
				Basistmp.push_back(WaveFuncsBasistmp[it][1]);
				Basistmp.push_back(mtmp);
				Basistmp.push_back(djtmp);
				WaveFuncsBasis.push_back(Basistmp);
				myfile1 << WaveFuncsBasistmp[it][0] << " " << WaveFuncsBasistmp[it][1] << " " << mtmp << " "<< djtmp << endl;
				size++;
				continue;
			}
			//if (FMagneticNumber) maxm=2*(l+1);
			int DMtmp=DM;
			if (ltmp<DM) DMtmp=ltmp;
			for (int m=ltmp-DMtmp; m<=ltmp+DMtmp+1; m++){
				for (int dj=-1;dj<=1;dj+=2){ 
						...
						WaveFuncsBasis.push_back(Basistmp);
						myfile1 << WaveFuncsBasistmp[it][0] << " " << WaveFuncsBasistmp[it][1] << " " << m << " "<< dj << endl;
						size++;
					}
				}
		}
	...
	string directory=to_string(size);
        ...
        cout << "Directory " << pchar <<" exists! EigenValue files will be written into it.\n Be careful not to overwrite previous data in this directory if you still need them." << endl;
        }
        else{
