      ...
      for (int m=0; m<maxm; m++){
        ...
        for (int dj=-1;dj<=1;dj+=2){
          if (l==0) dj+=2;
            ...
            string J=stream.str();
            name="../Wave/wfkt_X_Rb87_n="
               + to_string(n) +"_L=" + to_string(l) +"_J=" + J +"_step=0.001.txt";
            ifstream file;
            file.open(name);
            if (!file.good()){
              name="../Wave/wfkt_X_Rb87_n=" + to_string(n) +"_L=" 
                + to_string(l) +"_J=0_step=0.001.txt";
              file.open(name);
              if (!file.good()){
                cout << "Reading " << name << " failed! Check if file exists or not!"
                  << endl; 
