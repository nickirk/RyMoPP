...
int main(int argc, char const *argv[]) {
	int N=40, DN=3, DN1=0, DL=3,maxm=1;
	int size=0,n,l,j;
	double x=44.001, I;
	bool FReadH=false, FMagneticNumber=false, FPWave=true; // control the magnetic Number choices
	string name0;
	omp_set_nested(1);
		...
		for (int l=0; l<=DL;l++){
			...
			for (int m=0; m<maxm; m++){
				...
				for (int dj=-1;dj<=1;dj+=2){ 
					...
					cout << "Basis= " << WaveFuncsBasis[size][0] << endl;
					cout << "Main:n,l,m,dj=" << nt << "," << lt <<"," << mt << ","<<djt << endl;
					size++;

//					WaveFuncs.push_back(Radial0);		
//					cout << "Wave= " << WaveFuncs[size][1] << endl;
				}
			}
		}
	}
	...
	if(FReadH){
	  ...
	  SelfAdjointEigenSolver<MatrixXd> eigensolver(H);
		if (eigensolver.info() != Success){
			...
			ofstream myfile;
	  	myfile.open ("./090740SP/EigenValue"+to_string(x)+".txt");
